...
- (2.44.0, 完了): **c41s/c42s 専用研磨** `applySkillGroupConstraintPolish` を新設しフィックスポイント巡回に組込。
  c41/c42 と同型(判定が sgrp→ssk)。c41s は markNeed で needViolations 側に出て群情報が失われ職員セルから拾えないため、
  アンカーは `Problem.cons41s/cons42s` から直接算出（違反群 ssk の職員を起点）。同日2者スワップ(被覆/HARD不変)・`isBetter`採否
  (keep-best=退化なし)。SoftPolishVerify ログに c41s/c42s 増減と採用数(skill:n)を追加。
- (未): c41/c42(通常群)・c2・covO の専用研磨は未着手（c41/c42 も c41s/c42s と同型で同手法を流用可能だが別件・要指示）。
