...
| C | 画面反映（ホーム/勤務表/分析/編集/設定） | 🟡 ホーム✅ 勤務表(カレンダー/非色手がかり)✅ 分析(ゲージ)🟡 編集/設定🟡 |
...
### 5.5 設定 🟡
セクション毎に `MagiCard` + `MagiLabeledSlider`(実行時間/仮説数/研磨強度/並列) +
`Switch`(通知/片手) + 外観モード + ログ出力✅。外観の `MagiSegmentedControl` 化は⬜（長ラベル要検討）。
