...
- **構成**: 入力ガイドの下に `MagiSegmentedControl(['月次（毎月）','年次マスター'])`。月次=`WishCard`+`NeedDayCard`／年次=注意帯（「制度・人員が変わったときだけ編集」）＋`Ws1Card`+`ConstraintsCard`+`StaffRangeCard`。スキルグループ/新C41s/C42sのUIは年次側に置く（予定）。`MagiApp.kt` の `editScope`。
...
- **実装**: `ShiftPickerSheet`／`allowedShiftsFor`／`setCell`。
...
- **Web対応**: Web は ws3（希望グリッド）入力＋「そのまま反映」。Native は担当外を**確認ダイアログ＋Undo＋操作ログ**で安全化。年次マスター(ws4/ws5)に相当する専用画面は未対応（`SetupGuideCard`で手順提示まで）🟡。
