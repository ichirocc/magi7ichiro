...
- **Web対応**: Web は ws3（希望グリッド）入力＋「そのまま反映」。Native は担当外を**確認ダイアログ＋Undo＋操作ログ**で安全化。基本マスター(ws4 制約／ws5 個人回数)は編集タブ「基本マスター」に**専用エディタとして実装済み**（`ConstraintEditor`/`StaffRangeEditor` ほか。07b 参照）。
...
- **違反の内訳は全18種=100%**: 必須群（groupViol/pref/covU/c3n）＋人数の範囲群（low/high/apt）＋任意群（c1/c2/c3/c3m/c3mn/c41/c42/c41s/c42s/covO/**fair**）。直近で **fair（公平化のズレ）** を任意群に追加し 17/18→**18/18** に。fair はセル単位の場所を持たないため、内訳チップのタップ時は「場所情報がありません」（ペナルティ量は表示）。
