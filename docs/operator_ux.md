...
2. **専門用語を使わない**: §2 の言い換え表に従う。数字は必ず**言葉で意味**を添える（「78点（できあがり度）」）。
...
| 必須違反 / HARD | **守れていない約束**（例：夜勤の翌日に早番） |
| 調整 / SOFT | **できれば直したい点** |
| 人員不足 / covU | **人手が足りない日** |
| スタッフの希望 / pref | **スタッフの希望** |
...
| できた・配れる | **「③ できました！ そのまま配れます」**（緑） | **印刷・書き出し** | 中身を見る |
| できた・もう少し（人手不足あり） | **「もう少しです。○月○日が人手不足です」**（黄） | **なおすのを手伝って** | もう一度つくる |
| 直せない問題が残る | **「このデータでは、ここは埋められません」**（赤・理由つき） | **データを見直す** | そのまま配る |

- **常に主ボタンは1つだけ**を大きく。迷いを作らない。
- カードの下に「いまの出来ぐあい」を**言葉つきの数字**で（例：「人手が足りない日：1日 / できあがり度：78点」）。
...
- **数字は言葉つき**: 「78点（できあがり度）」「1日（人手が足りない日）」。
...
2. ✅ 用語の**言い換え適用**（§2）。次アクションカードに加え、**ホームの主要面に適用済み**：状態見出し(`StatusHero`：配れます／守れていない約束／まだ作っていません)、操作アシスト(`CopilotCard`：できあがり度／コンピューターが組んでいます)、実行カード(`ActionCard`：下書きをつくる／勤務表をつくる／閉じても大丈夫、反復/iter/s は非表示で経過秒のみ)、下部CTA(`BottomCommandBar`：勤務表をつくる／もう一度つくる／やめる)、データ(問題がないか調べる)。分析タブの一部技術語（人員充足率 等）は設計上据え置き。
